...
```{r}
table(songAppData$DeviceOS, songAppData$DeviceOS>0)
```
...
```{r}
prop.table(table(songAppData$DeviceOS, songAppData$DeviceOS>0))
```
...
```{r, eval=FALSE} 
plot(sessionsStartedEachHour$X.1 ~ sessionsStartedEachHour$times)
...
```{r}
quantile(songAppData$avgLatencies, c(0.5,0.95))
```
